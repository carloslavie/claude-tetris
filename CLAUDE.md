...
- **Cell values are piece types.** `PIECES[type]` matrices store the type index (1–7) in their filled cells, and that same number indexes `COLORS`. `merge()` copies those values into `board`, so the board holds color indices, not booleans. Index 0 / `null` means empty. A new piece needs matching entries in both arrays, and `randomPiece()` hardcodes `* 7`.
